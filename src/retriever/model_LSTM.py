...
class RL_LSTM(nn.Module):
    #初始化基于LSTM的问答模型：RoBERTa文本编码器->预训练知识图谱嵌入->LSTM序列处理器->ComplEx评分函数
    ...
    entdrop=0.0, reldrop=0.0, scoredrop=0.0, l3_reg=0.0, ls=0.0, do_batch_norm=True):
        ...
        self.roberta_model = RobertaModel.from_pretrained("D:/Project/ConvQA/model/roberta-base")
    ...
    def applyNonLinear(self, outputs):#将LSTM的输出映射到关系嵌入空间，文本表示转换成KG中的关系表示
    ...
    #问题嵌入提取：使用RoBERTa模型提取问题语义表示，将自然语言问题转换成固定维度是向量
    ...
    def forward(self, p_head, question_tokenized1, attention_mask1, p_tail1,
        ...
        # output shoule be the same as h, check   LSTM序列处理
